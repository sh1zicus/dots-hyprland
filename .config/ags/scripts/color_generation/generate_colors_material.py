...
from materialyoucolor.utils.color_utils import (
    rgba_from_argb, argb_from_rgb, argb_from_rgba)
...
parser.add_argument('--color', type=str, default=None,
                    help='generate colorscheme from color')
parser.add_argument('--mode', type=str,
                    choices=['dark', 'light'], default='dark', help='dark or light mode')
parser.add_argument('--scheme', type=str, default='vibrant',
                    help='material scheme to use')
parser.add_argument('--smart', action='store_true', default=False,
                    help='decide scheme type based on image color')
parser.add_argument('--transparency', type=str, choices=[
                    'opaque', 'transparent'], default='opaque', help='enable transparency')
parser.add_argument('--termscheme', type=str, default=None,
                    help='JSON file containg the terminal scheme for generating term colors')
parser.add_argument('--harmony', type=float, default=0.8,
                    help='(0-1) Color hue shift towards accent')
parser.add_argument('--harmonize_threshold', type=float, default=100,
                    help='(0-180) Max threshold angle to limit color hue shift')
parser.add_argument('--term_fg_boost', type=float, default=0.35,
                    help='Make terminal foreground more different from the background')
parser.add_argument('--blend_bg_fg', action='store_true', default=False,
                    help='Shift terminal background or foreground towards accent')
parser.add_argument('--cache', type=str, default=None,
                    help='file path to store the generated color')
parser.add_argument('--debug', action='store_true',
                    default=False, help='debug mode')
args = parser.parse_args()


def rgba_to_hex(rgba): return "#{:02X}{:02X}{:02X}".format(
    rgba[0], rgba[1], rgba[2])


def argb_to_hex(argb): return "#{:02X}{:02X}{:02X}".format(
    *map(round, rgba_from_argb(argb)))


def hex_to_argb(hex_code): return argb_from_rgb(
    int(hex_code[1:3], 16), int(hex_code[3:5], 16), int(hex_code[5:], 16))


def display_color(rgba): return "\x1B[38;2;{};{};{}m{}\x1B[0m".format(
...
def calculate_optimal_size(width: int, height: int, bitmap_size: int) -> (int, int):
    ...
    scale = math.sqrt(
        bitmap_area/image_area) if image_area > bitmap_area else 1
...
if args.path is not None:
    ...
    # Get pixel data and ensure it's a list of RGB tuples
    pixels = list(image.getdata())
    pixels_rgb = [(pixel[0], pixel[1], pixel[2]) for pixel in pixels]  # Extract RGB channels

    # Pass the processed pixel data to QuantizeCelebi
    colors = QuantizeCelebi(pixels_rgb, 128)
...
if args.termscheme is not None:
    ...
    term_source_colors = json.loads(json_termscheme)[
        'dark' if darkmode else 'light']

    primary_color_argb = hex_to_argb(
        material_colors['primary_paletteKeyColor'])
    for color, val in term_source_colors.items():
        ...
        if args.blend_bg_fg and color == "term0":
            harmonized = boost_chroma_tone(hex_to_argb(
                material_colors['surfaceContainerLow']), 1.2, 0.95)
        elif args.blend_bg_fg and color == "term15":
            harmonized = boost_chroma_tone(
                hex_to_argb(material_colors['onSurface']), 3, 1)
        else:
            harmonized = harmonize(hex_to_argb(
                val), primary_color_argb, args.harmonize_threshold, args.harmony)
            harmonized = boost_chroma_tone(
...
if args.debug == False:
    ...
    print(f"Accent color: {display_color(
        rgba_from_argb(argb))} {argb_to_hex(argb)}")
    ...
    for color, code in term_colors.items():
        ...
        print(f"{color.ljust(6)} : {display_color(rgba_source)} {
              code_source} --> {display_color(rgba)} {code}")
    print('-----------------------------------------------')
