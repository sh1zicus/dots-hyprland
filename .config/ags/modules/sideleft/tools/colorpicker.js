...
const { Box, Button, Entry, EventBox, Icon, Label, Overlay, Scrollable } =
  Widget;
...
export default () => {
  ...
  function shouldUseBlackColor() {
    return (
      (selectedColor.xAxis < 40 ||
        (45 <= selectedColor.hue && selectedColor.hue <= 195)) &&
    ...
    children: [
      Box({
        ...
        attribute: {
          update: (self) => {
            const allocation =
              saturationAndLightnessRange.children[0].get_allocation();
            self.setCss(`
                        margin-left: ${(13.636 * selectedColor.xAxis) / 100}rem;
                        margin-top: ${(13.636 * (100 - selectedColor.yAxis)) / 100}rem;
                    `); // Why 13.636rem? see class name in stylesheet
  ...
  const resultColorBox = Box({
    ...
    attribute: {
      update: (self) => {
        ...
        self.children[0].setCss(
          `color: ${shouldUseBlackColor() ? colorBlack : colorWhite};`,
        );
      },
    },
    setup: (self) =>
  ...
  const ResultBox = ({ colorSystemName, updateCallback, copyCallback }) =>
    Box({
      children: [
        Box({
          vertical: true,
  ...
  const resultRgb = ResultBox({
    ...
    copyCallback: () =>
      Utils.execAsync([
        "wl-copy",
        `rgb(${hslToRgbValues(selectedColor.hue, selectedColor.xAxis, selectedColor.yAxis / (1 + selectedColor.xAxis / 100))})`,
      ]),
  });
  const resultHsl = ResultBox({
    ...
    copyCallback: () =>
      Utils.execAsync([
        "wl-copy",
        `hsl(${selectedColor.hue},${selectedColor.xAxis}%,${Math.round(selectedColor.yAxis / (1 + selectedColor.xAxis / 100))}%)`,
      ]),
  });
  const result = Box({
