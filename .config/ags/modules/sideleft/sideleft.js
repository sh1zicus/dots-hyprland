    ...
    setup: (self) =>
      self.on("key-press-event", (widget, event) => {
        ...
        if (checkKeybind(event, userOptions.asyncGet().keybinds.sidebar.pin))
          ...
          checkKeybind(event, userOptions.asyncGet().keybinds.sidebar.cycleTab)
        )
          widgetContent.cycleTab();
        else if (
          checkKeybind(event, userOptions.asyncGet().keybinds.sidebar.nextTab)
        )
          widgetContent.nextTab();
        else if (
          checkKeybind(event, userOptions.asyncGet().keybinds.sidebar.prevTab)
        )
        ...
        if (
          ...
          // Switch API type
            ...
            checkKeybind(
              event,
              userOptions.asyncGet().keybinds.sidebar.apis.nextTab,
            )
          ) {
            const toSwitchTab =
            ...
            checkKeybind(
              event,
              userOptions.asyncGet().keybinds.sidebar.apis.prevTab,
            )
          ) {
            const toSwitchTab =
              widgetContent.attribute.children[
