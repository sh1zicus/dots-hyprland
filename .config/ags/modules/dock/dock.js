...
const icon_files = userOptions
    .asyncGet()
    .icons.searchPaths.map((e) => getAllFiles(e))
    .flat(1);
...
const Rofi = () =>
    Widget.Button({
        ...
        child: Widget.Box({
            homogeneous: true,
            className: "dock-app-icon txt",
            child: MaterialIcon("view_cozy", "massive"),
        }),
        onClicked: () => {
            Utils.execAsync([
                "bash",
    ...
    Widget.Box({
        ...
        setup: (self) => {
            self.hook(
                ...
                Hyprland,
                (box, address) =>
                    box.attribute.remove(box, address, self.monitor),
                "client-removed",
            );
...
const PinnedApps = () =>
    Widget.Box({
        ...
        children: userOptions
            .asyncGet()
            .dock.pinnedApps.map((term) => ({
                app: Applications.query(term)?.[0],
                term,
            }))
            .filter(({ app }) => app)
            .map(({ app, term = true }) => {
    ...
    const dockRevealer = Revealer({
        ...
        setup: (self) => {
            const callback = (self, trigger) => {
                if (!userOptions.asyncGet().dock.trigger.includes(trigger))
                    return;
