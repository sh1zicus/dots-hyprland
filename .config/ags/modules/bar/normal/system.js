      ...
      try {
        ...
        const bgFiles = (
          await Utils.execAsync(
            `find ${bgFolder} -type f -iname '*.png' -o -iname '*.jpg'`,
          )
        ).split("\n");
        const bgFile =
          bgFiles[Math.floor(Math.random() * (bgFiles.length - 1))];
        await Utils.execAsync(
          `sh ${Utils.HOME}/.config/ags/scripts/color_generation/switchwall.sh ${bgFile}`,
        );
      } catch (e) {
        console.error(e);
      }
