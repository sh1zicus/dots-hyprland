        ...
        attribute: {
            ...
            updateMask: (self) => {
                const offset =
                    ...
                    userOptions.asyncGet().workspaces.shown;
            ...
            toggleMask: (self, occupied, name) => {
                if (occupied)
                    self.attribute.workspaceMask |= 1 << parseInt(name);
            ...
            area
                ...
                .on(
                    ...
                    Lang.bind(area, (area, cr) => {
                        ...
                        const workspaceStyleContext =
                            dummyWs.get_style_context();
                        const workspaceDiameter =
                        ...
                        const occupiedbg =
                            occupiedWorkspaceStyleContext.get_property(
                                "background-color",
                                Gtk.StateFlags.NORMAL,
                            );
                        const occupiedfg =
                        ...
                        layout.set_text(
                            "0".repeat(count.toString().length),
                            -1,
                        );
                        ...
                        for (let i = 1; i <= count; i++) {
                            if (area.attribute.workspaceMask & (1 << i)) {
                                ...
                                const wsCenterX =
                                    -workspaceRadius + workspaceDiameter * i;
                                const wsCenterY = height / 2;
                                if (
                                    !(
                                        area.attribute.workspaceMask &
                                        (1 << (i - 1))
                                    )
                                ) {
                                    // Left
                        ...
                        for (let i = 1; i <= count; i++) {
                            const inactivecolors =
                                area.attribute.workspaceMask & (1 << i)
                                    ? occupiedfg
                                    : wsfg;
                            if (i == activeWs) {
                                cr.setSourceRGBA(
        ...
        setup: (self) => {
            ...
            self.on("motion-notify-event", (self, event) => {
                ...
                const wsId = Math.ceil(
                    (cursorX * userOptions.asyncGet().workspaces.shown) /
                        widgetWidth,
                );
                Utils.execAsync([
