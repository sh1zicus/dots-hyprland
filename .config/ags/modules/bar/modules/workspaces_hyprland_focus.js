    ...
    attribute: {
      ...
      updateMask: (self) => {
        const offset =
          ...
          userOptions.asyncGet().workspaces.focus;
    ...
    child: Box({
      ...
      children: [
        Box({
          ...
          css: "min-width: 2px;",
          children: [
            WorkspaceContents(userOptions.asyncGet().workspaces.focus),
          ],
        }),
      ],
    }),
    setup: (self) => {
      ...
      self.on("motion-notify-event", (self, event) => {
        ...
        const wsId = Math.ceil(
          (cursorX * userOptions.asyncGet().workspaces.focus) / widgetWidth,
        );
        Utils.execAsync([
