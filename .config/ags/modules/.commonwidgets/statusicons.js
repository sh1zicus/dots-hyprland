            ...
            children: [
                ...
                Widget.Label({
                    ...
                    attribute: {
                        unreadCount: 0,
                        update: (self) =>
                            (self.label = `${self.attribute.unreadCount}`),
                    },
                    setup: (self) =>
                        self
                            .hook(
...
const HyprlandXkbKeyboardLayout = async ({ useFlag } = {}) => {
    try {
        ...
        const updateCurrentKeyboards = () => {
            ...
            languageStackArray = Array.from(
                { length: initLangs.length },
                (_, i) => {
                    const lang = languages.find(
                        (lang) => lang.layout == initLangs[i],
                    );
                    if (!lang)
                        return {
                            [initLangs[i]]: Widget.Label({
                                label: initLangs[i].toUpperCase(),
                            }),
                        };
                    return {
